...
object CodeHighlighter {
    ...
    private fun highlightJson(code: String, theme: SyntaxTheme): AnnotatedString {
        return buildAnnotatedString {
            ...
            propertyRegex.findAll(code).forEach { match ->
                addStyle(SpanStyle(color = theme.property), match.range.first, match.range.first + match.groupValues[0].indexOf(':'))
            }
        }
    }
    ...
    private fun highlightXml(code: String, theme: SyntaxTheme): AnnotatedString {
        return buildAnnotatedString {
            ...
            tagNameRegex.findAll(code).forEach { match ->
                ...
                addStyle(SpanStyle(color = theme.type, fontWeight = FontWeight.SemiBold), nameStart, nameStart + tagName.length)
            }
            ...
            bracketRegex.findAll(code).forEach { match ->
                addStyle(SpanStyle(color = theme.punctuation), match.range.first, match.range.last + 1)
            }
        }
    }
    ...
    private fun highlightKotlin(code: String, theme: SyntaxTheme): AnnotatedString {
        ...
        return buildAnnotatedString {
            ...
            wordRegex.findAll(code).forEach { match ->
                ...
                when {
                    word in keywords -> {
                        addStyle(SpanStyle(color = theme.keyword, fontWeight = FontWeight.SemiBold), match.range.first, match.range.last + 1)
                    }
                    word in types -> {
                        addStyle(SpanStyle(color = theme.type), match.range.first, match.range.last + 1)
                    }
                    word[0].isUpperCase() -> {
                        addStyle(SpanStyle(color = theme.type), match.range.first, match.range.last + 1)
                    }
            ...
            functionRegex.findAll(code).forEach { match ->
                ...
                addStyle(SpanStyle(color = theme.function), nameStart, nameStart + functionName.length)
            }
        }
    }
    ...
    private fun highlightCss(code: String, theme: SyntaxTheme): AnnotatedString {
        return buildAnnotatedString {
            ...
            propertyRegex.findAll(code).forEach { match ->
                ...
                if (nameStart >= 0) {
                    addStyle(SpanStyle(color = theme.property), nameStart, nameStart + propName.length)
                }
            }
        ...
        return buildAnnotatedString {
            ...
            keywords.forEach { keyword ->
                ...
                regex.findAll(code).forEach { match ->
                    addStyle(SpanStyle(color = theme.keyword, fontWeight = FontWeight.Bold), match.range.first, match.range.last + 1)
                }
            }
    ...
    private fun highlightYaml(code: String, theme: SyntaxTheme): AnnotatedString {
        return buildAnnotatedString {
            ...
            keyRegex.findAll(code).forEach { match ->
                ...
                if (keyStart >= 0) {
                    addStyle(SpanStyle(color = theme.property, fontWeight = FontWeight.SemiBold), keyStart, keyStart + key.length)
                }
            }
            ...
            code.lines().forEach { line ->
                when {
                    line.startsWith("+") && !line.startsWith("+++") -> {
                        appendLine()
                        addStyle(SpanStyle(color = Color(0xFF22863A), background = Color(0xFFE6FFED)), length - line.length - 1, length - 1)
                        append(line)
                    }
                    line.startsWith("-") && !line.startsWith("---") -> {
                        appendLine()
                        addStyle(SpanStyle(color = Color(0xFFCB2431), background = Color(0xFFFFEBED)), length - line.length - 1, length - 1)
                        append(line)
                    }
                    line.startsWith("@@") -> {
                        appendLine()
                        addStyle(SpanStyle(color = theme.type, fontWeight = FontWeight.Bold), length - line.length - 1, length - 1)
                        append(line)
                    }
                    else -> {
                        appendLine()
    ...
    private fun highlightWithKeywords(code: String, theme: SyntaxTheme, keywords: Set<String>): AnnotatedString {
        return buildAnnotatedString {
            append(code)
